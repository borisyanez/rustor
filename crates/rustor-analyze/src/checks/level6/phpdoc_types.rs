...
impl<'s> PhpDocTypesVisitor<'s> {
    ...
    fn validate_stmt_phpdocs<'a>(&mut self, stmt: &Statement<'a>) {
        match stmt {
            Statement::Function(func) => {
                // Collect @template params and @phpstan-type aliases for this function
                let func_offset = func.span().start.offset as usize;
                self.collect_type_aliases_from_doc(func_offset);
                let phpdoc = self.extract_phpdoc(func_offset);
                if let Some(ref doc) = phpdoc {
                    for template in &doc.templates {
                        self.template_params.insert(template.name.to_lowercase());
                    }
                    self.validate_phpdoc(doc, func_offset);
                }
                self.template_params.clear();
            }
            Statement::Class(class) => {
                // Collect class-level @template params and @phpstan-type aliases
                let class_offset = class.span().start.offset as usize;
                self.collect_type_aliases_from_doc(class_offset);
                let class_phpdoc = self.extract_phpdoc(class_offset);
                if let Some(ref doc) = class_phpdoc {
                    for template in &doc.templates {
                        self.template_params.insert(template.name.to_lowercase());
                    }
                    self.validate_phpdoc(doc, class_offset);
                }
    ...
    /// Extract @phpstan-type and @psalm-type aliases from a doc comment and register
    /// them as known type names (similar to @template params)
    fn collect_type_aliases_from_doc(&mut self, offset: usize) {
        // Find the doc comment before this offset
        let before = &self.source[..offset];
        if let Some(doc_end) = before.rfind("*/") {
            if let Some(doc_start) = before[..doc_end].rfind("/**") {
                let doc_comment = &self.source[doc_start..doc_end + 2];
                // Look for @phpstan-type Name or @psalm-type Name
                for line in doc_comment.lines() {
                    let trimmed = line.trim().trim_start_matches('*').trim();
                    if let Some(rest) = trimmed.strip_prefix("@phpstan-type ")
                        .or_else(|| trimmed.strip_prefix("@psalm-type "))
                        .or_else(|| trimmed.strip_prefix("@phpstan-import-type "))
                        .or_else(|| trimmed.strip_prefix("@psalm-import-type "))
                    {
                        // Format: TypeName = ... or TypeName from ClassName
                        let type_name = rest.split_whitespace().next().unwrap_or("");
                        if !type_name.is_empty() {
                            self.template_params.insert(type_name.to_lowercase());
                        }
                    }
                }
            }
    ...
    fn validate_type(&mut self, ty: &Type, offset: usize) {
        ...
        for class_name in class_names {
            // Clean up malformed class names (trailing backslash, semicolons, backticks, etc.)
            let cleaned_name = class_name.trim_end_matches(|c: char| {
                c == '\\' || c == ';' || c == '`' || c == ',' || c == '.'
                    || c == ':' || c == '(' || c == ')' || c == '{' || c == '}'
            });
            if cleaned_name.is_empty() || cleaned_name.contains("():") {
                continue;
            }
    ...
    fn is_class_defined(&self, name: &str) -> bool {
        ...
        if is_absolute_fqn {
            ...
            // Check builtin classes with normalized name (without leading backslash)
