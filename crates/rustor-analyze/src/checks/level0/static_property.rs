    ...
    fn class_has_property(&self, class_name: &str, property_name: &str) -> (bool, bool) {
        ...
        if let Some(st) = self.symbol_table {
            if let Some(class_info) = st.get_class(&class_lower) {
                // If methods weren't fully collected (e.g. classmap-only stub), be conservative
                if !class_info.methods_fully_collected {
                    return (true, true);
                }

                // Check direct property (symbol table stores with original case)
                if class_info.has_property(property_name) {
                    return (true, true);
                }
                ...
                while let Some(ref parent_name) = current_parent {
                    ...
                    if let Some(parent_info) = st.get_class(&parent_name.to_lowercase()) {
                        if parent_info.has_property(property_name) {
                            return (true, true);
                        }
                ...
                while let Some(trait_name) = traits_to_check.pop() {
                    ...
                    if let Some(trait_info) = st.get_class(&trait_lower) {
                        if trait_info.has_property(property_name) {
                            return (true, true);
                        }
