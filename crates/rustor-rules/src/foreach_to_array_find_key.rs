//! Rule: Convert foreach loops to array_find_key() (PHP 8.4+)
//!
//! Converts foreach loops that find the key of the first matching element to array_find_key().
//!
//! Pattern:
//! ```php
//! // Before
...
//!
//! // After
//! $foundKey = array_find_key($animals, fn($animal) => str_starts_with($animal, 'c'));
//! ```
...
/// Check a parsed PHP program for foreach to array_find_key conversions
pub fn check_foreach_to_array_find_key<'a>(program: &Program<'a>, source: &str) -> Vec<Edit> {
    let mut checker = ForeachToArrayFindKeyChecker {
        source,
        edits: Vec::new(),
    };
...
struct ForeachToArrayFindKeyChecker<'s> {
    source: &'s str,
    edits: Vec<Edit>,
}

impl<'s> ForeachToArrayFindKeyChecker<'s> {
    ...
    fn check_statement_sequence(&mut self, statements: &[Statement<'_>]) {
        ...
        for i in 0..statements.len() {
            // Pattern: $foundKey = null; foreach ($arr as $key => $item) { if (cond) { $foundKey = $key; break; } }
            if i > 0 {
    ...
    fn check_null_assignment_pattern(
        ...
        let Statement::Block(block) = first_stmt else {
            return self.check_find_key_foreach_body(first_stmt, foreach, key_value, &var_name, prev_stmt);
        };
        ...
        self.check_find_key_foreach_body(&block_stmts[0], foreach, key_value, &var_name, prev_stmt)
    }

    fn check_find_key_foreach_body(
        &self,
        stmt: &Statement<'_>,
        foreach: &Foreach<'_>,
        key_value: &ForeachKeyValueTarget<'_>,
        var_name: &str,
        prev_stmt: &Statement<'_>,
    ) -> Option<Edit> {
        ...
        // First must be $var = $key (the foreach key variable)
        ...
        // The assigned value must be the foreach KEY variable (not value)
        ...
        Some(Edit::new(
            span,
            replacement,
            "Convert foreach to array_find_key() (PHP 8.4+)",
        ))
    }
...
pub struct ForeachToArrayFindKeyRule;

impl Rule for ForeachToArrayFindKeyRule {
    fn name(&self) -> &'static str {
        "foreach_to_array_find_key"
    }

    fn description(&self) -> &'static str {
        "Convert foreach loops finding first matching key to array_find_key() (PHP 8.4+)"
    }

    fn check<'a>(&self, program: &Program<'a>, source: &str) -> Vec<Edit> {
        check_foreach_to_array_find_key(program, source)
    }
...
"#;
        ...
        assert!(result.contains("$foundKey = array_find_key($animals, fn($animal) => str_starts_with($animal, 'c'))"));
    }
...
"#;
        ...
        assert_eq!(edits.len(), 1);
        let result = transform(source);
        assert!(result.contains("$resultKey = array_find_key($items, fn($item) => $item > 10)"));
    }

    #[test]
    fn test_skip_no_key_value() {
        // No key in foreach - should not match (use array_find instead)
        let source = r#"<?php
$foundKey = null;
...
"#;
    ...
    #[test]
    fn test_skip_assign_value_not_key() {
        // Assigns the value instead of the key
        let source = r#"<?php
$found = null;
foreach ($animals as $key => $animal) {
    if (str_starts_with($animal, 'c')) {
...
"#;
        ...
        assert_eq!(edits.len(), 1);
        let result = transform(source);
        assert!(result.contains("$index = array_find_key($users, fn($user) => $user->isActive())"));
    }
}
